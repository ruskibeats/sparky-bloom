# Privacy Policy for SparkyFitness

**Last Updated: [Date]**

## Introduction
SparkyFitness ("we," "our," or "us") is committed to protecting your privacy. This Privacy Policy explains how our mobile application (the "App") collects, uses, and safeguards your information.

## Health Data
SparkyFitness integrates with Apple HealthKit and Google Health Connect to provide you with fitness insights.

### What We Collect
With your explicit permission, we may read the following data from your device's health database:
- Active Energy Burned
- Basal Metabolic Rate
- Steps
- Workouts
- [List other specific types used]

We may also write data (such as workout sessions) to these services if you use tracking features within the App.

### How We Use Health Data
- **Display**: To show you your fitness progress and metrics within the App.
- **Analysis**: To calculate daily summaries (e.g., Total Calories).
- **Storage**: Your health data is primarily processed locally on your device. [If you sync to a server, disclose it here: "We may sync strictly necessary data to our secure servers for backup and cross-device functionality."]

### Data Sharing
**We do not sell your personal data or health data to third parties.** We do not use your health data for advertising purposes.

## Security
We implement reasonable security measures to protect your information. However, no method of transmission over the internet or electronic storage is 100% secure.

## Your Choices
You can revoke the App's access to HealthKit or Health Connect at any time through your device settings.

## Contact Us
If you have questions about this Privacy Policy, please contact us at: [Your Support Email]
