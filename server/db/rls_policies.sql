...
CREATE POLICY t1d_onboarding_data_select_policy ON public.t1d_onboarding_data FOR SELECT TO PUBLIC
USING (
  t1d_profile_id IN (
    SELECT id FROM public.t1d_profiles
    WHERE sparky_user_id = current_user_id()
  )
);
CREATE POLICY t1d_onboarding_data_insert_policy ON public.t1d_onboarding_data FOR INSERT TO PUBLIC
WITH CHECK (
  t1d_profile_id IN (
    SELECT id FROM public.t1d_profiles
    WHERE sparky_user_id = current_user_id()
...
CREATE POLICY t1d_onboarding_data_update_policy ON public.t1d_onboarding_data FOR UPDATE TO PUBLIC
USING (
  t1d_profile_id IN (
    SELECT id FROM public.t1d_profiles
    WHERE sparky_user_id = current_user_id()
